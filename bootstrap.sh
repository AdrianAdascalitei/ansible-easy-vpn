#!/bin/bash -uxe
# A bash script that prepares the OS before running the Ansible playbook
...
if grep -qs "ubuntu" /etc/os-release; then
    ...
    echo "Unsupported OS. This script supports Ubuntu, Debian, and CentOS."
    exit 1
fi
...
if [[ "$adguard_enable" =~ ^[yY]$ ]]; then
    echo "Make sure that 'w', 'auth-w' and 'adguard' subdomains also point to that IP (not necessary with DuckDNS)"
else
    echo "Make sure that 'auth-w' and 'w' subdomains also point to that IP (not necessary with DuckDNS)"
fi
echo
...
if [[ "$adguard_enable" =~ ^[yY]$ ]]; then
    $SUDO .venv/bin/certbot certonly --non-interactive --break-my-certs --force-renewal --agree-tos --email root@localhost.com --standalone --staging -d $root_host -d w.$root_host -d auth-w.$root_host -d adguard.$root_host || $SUDO .venv/bin/certbot certonly --non-interactive --force-renewal --agree-tos --email root@localhost.com --standalone -d $root_host -d w.$root_host -d auth-w.$root_host -d adguard.$root_host || exit
else
    $SUDO .venv/bin/certbot certonly --non-interactive --break-my-certs --force-renewal --agree-tos --email root@localhost.com --standalone --staging -d $root_host -d w.$root_host -d auth-w.$root_host || $SUDO .venv/bin/certbot certonly --non-interactive --force-renewal --agree-tos --email root@localhost.com --standalone -d $root_host -d w.$root_host -d auth-w.$root_host || exit
fi
echo "OK"
